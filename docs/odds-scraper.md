...
| Slate CLI | `scrape/slate.php board\|sql …` | Read-only helper for the `/pick-games` skill ([.claude/skills/pick-games/SKILL.md](../.claude/skills/pick-games/SKILL.md)), which chooses the week's 14 lines instead of the admin ticking them. `board` prints the newest scrape per league with AP ranks (`--rankings` JSON) and ESPN FPI ranks (`--fpi` JSON, the owner's measure of a big game), Central time slot, eligibility under the pool's rules and a big-game ordering; `sql --week=N --slate=FILE` turns picks keyed `LEAGUE:away@home:spread\|over-under` into the same `football_games` `INSERT` the admin page would write, with rule warnings on stderr. Added 2026-09-25. |
