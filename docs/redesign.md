...
As built: `?pool` absent selects the viewer's pool (as the classic page does), `pool=0` everyone; the default week is the active (else latest) season's latest week with visible results, computed from one grouped games query (not `Season::getResultWeek()`, which returns the earliest, see section 9). Money is the classic page's: overall `WeekResults` for payouts, a pool-only one for a pool's ranks and probabilities, `WeekPayouts::syncWinners` once complete, winnings from `football_week_winners`; probability columns follow the classic `num_winners`/threshold/pools rules. Header: week picker (prev/next visible week and a menu of the season's weeks), payout chips grouped as `WeekFormatPayouts::groups`, a live status line ("N in play · N final · N to come"), a "Reset N what-ifs" pill, and the pool `.seg` (what-ifs carry across pools). Standings sort by header (`aria-sort`), pin rank and player on phones, and show a "me" bar at the bottom while the standings are on screen but the viewer's row is not. Games are grouped by day; each side shows pick count and point sum, pickers (mine outlined, friends marked, collapsed after five), right/wrong tint, striped what-ifs, a Leading marker while undecided, and What if / Undo links that navigate in place keeping scroll. The grid shows 10→1 with ✓ ✗ ◎ ?, top 10 plus me. The chart loads the classic `static/js/chart.js` on demand, uses theme tokens and redraws on `p55:theme`. `results.js` polls `api/week.php` every 60 s while a game is in play (300 s otherwise), repaints scores, statuses, leading markers, the status line and points/right/Exp $ with `P55.flash`, FLIP-animates rank changes while the table is in rank order, and calls `P55.reload()` when a result is set or the week completes. The recap moment adds "Week in one line": winner(s) and their points, my finish and points, my winnings (no "biggest swing"). Access failures show `Shell::empty` rather than redirecting.
