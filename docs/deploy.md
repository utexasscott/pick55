...
| PHP | **Apache runs PHP 7.4** (`mods-enabled/php7.4`). **The CLI `php` is 8.0.10.** Every version 5.6–8.0 is installed under `/etc/php`. CLI work (cron, scripts) uses 8.0: the 7.4 CLI build has no `dom` extension and warns on `pdo_mysql` at startup (measured 2026-09-25), while `/usr/bin/php` (8.0) has `dom`, `curl`, `mysqli`, `mysqlnd`, `pdo_mysql`. Code must stay valid on both. |
